...
const settle_post = async (req, res) => {
  if (req.body.task === "GET SETTLES") {
    ...
    const inserted_id = await insert_button_table(
      group_id,
      give_id,
      take_id,
      amount
    ...
    let give_mail_id = await connection.execute(
      `SELECT user_email FROM MAIN_users WHERE user_id = ${give_id}`
    );
    give_mail_id = give_mail_id[0][0]["user_email"];

    let take_mail_id = await connection.execute(
      `SELECT user_email FROM MAIN_users WHERE user_id = ${take_id}`
    );
...
async function get_result(user_id, group_id) {
  ...
  let result = await connection.execute(
    `SELECT user_id , spent-paid as DIFF FROM ${table_name}`
  );
  result = result[0];
  //console.log(result)
...
function get_amount(expense, ids, user_id) {
  ...
  while (start < end) {
    ...
    if (Math.abs(expense[start]) < Math.abs(expense[end])) {
      ...
      if (ids[end] == user_id || ids[start] == user_id) mat.push(json);
...
async function get_mails(result) {
  ...
  for (i = 0; i < result.length; i++) {
    let ans = await connection.execute(
      `SELECT user_email FROM MAIN_users WHERE user_id=${result[i].give_id}`
    );
    result[i]["give_mail"] = ans[0][0]["user_email"];

    ans = await connection.execute(
      `SELECT user_email FROM MAIN_users WHERE user_id=${result[i].take_id}`
    );
    console.log(ans[0][0])
...
async function insert_button_table(group_id, give_id, take_id, amount) {
  ...
  let result = await connection.execute(
    `INSERT INTO ${table_name} (take_id,give_id,amount) VALUES (${take_id},${give_id},${amount})`
  );
  ...
  query = `UPDATE ${table_name} SET paid = paid + ${amount} WHERE user_id=${give_id}`;
  console.log(query);
