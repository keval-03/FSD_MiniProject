...
const home_post = async (req, res) => {
    ...
    // logs_[group_id] table  --> by_id , for_id , logs , dtime
        ...
        for (i = 0; i < member_mails.length; i++) {
            const [rows, cols] = await connection.query('CALL get_user_id(?)', [member_mails[i]])
        ...
        let query = 'CREATE TABLE ' + table_name + ' (user_id INTEGER , paid INTEGER, spent INTEGER,FOREIGN KEY (user_id) REFERENCES MAIN_users(user_id) )';
        ...
        query = 'CREATE TABLE ' + table_name + ' (by_id INTEGER, for_id INTEGER, amount INTEGER, state TINYINT, FOREIGN KEY (by_id) REFERENCES MAIN_users(user_id), FOREIGN KEY (for_id) REFERENCES MAIN_users(user_id) )'
        //console.log(query)
